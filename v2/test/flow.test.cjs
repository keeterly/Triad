...
(async () => {
  ...
  check('BOSS: both blows feed the row-threat telegraph (front + mid lit)',
    await J(() => {
      // intents[0]=front, intents[1]=buff → next two from idx 0 are front + buff;
      // set idx 2 so the pair is mid + back (two distinct damaging rows)
      S.enemies[0].intentIdx = 2; renderAll();
