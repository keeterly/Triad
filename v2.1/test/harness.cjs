...
async function boot(opts = {}) {
  ...
  await page.addInitScript((init) => {
    try {
      localStorage.setItem('kizuna2_1.unlocked', '1');
      ...
      if (!sessionStorage.getItem('__v2_1init')) {
        ...
        if (init.flow != null) localStorage.setItem('kizuna2_1.flow', String(init.flow));
        if (init.clearRun) { localStorage.removeItem('kizuna2_1.run'); localStorage.removeItem('kizuna2_1.abyss'); localStorage.removeItem('kizuna2_1.vows'); }
      }
    } catch (_) {}
  ...
  await page.goto(`http://127.0.0.1:${port}/v2.1/index.html`, { waitUntil: 'networkidle' });
