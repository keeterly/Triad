// Hand-play one whole turn, card by card, with the EP showing — so a number from
// a meter is never the first time anyone has seen what the change actually does.
// Written because the relaymeter reported the relay playing MORE cards a turn,
// which is the opposite of what the design predicted, and a surprising number in
// this repo gets played by hand before it gets reported.
//
//   node test/probe-relay.cjs            the relay
//   RELAY=0 node test/probe-relay.cjs    the private chains it replaces
'use strict';
...
(async () => {
  ...
  await t.J((relay) => {
    ...
    RUN.bonds = {}; RUN._rotations = true; RUN._relay = relay;
    ...
    S._rotations = true; S._relay = relay; renderAll();
  ...
  console.log('\n' + (RELAY ? '=== THE RELAY ===' : '=== PRIVATE CHAINS ==='));
  ...
  console.log('\ncards this turn: ' + n + '   ' + await t.J(() =>
    'ep left ' + S.ep + '  relay ' + JSON.stringify(S.relay) + '  foe hp lost '
